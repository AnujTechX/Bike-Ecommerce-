...
const Scooters = ({group}) => {
  ...
  <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
    ...
    <div className="mb-4 mx-2 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3">
       ...
       {   
    currGroup &&
        currGroup.slice(0,visible).map((item)=>
         <ScooterCards item={item} key={item.id}/>
      )
     }

  </div>
    {visible < currGroup.length && (
    <div className="w-full text-center">
      <button onClick={()=>setVisible(prev=> prev+currGroup.length)}
        type="button"
        className="cursor-pointer rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
      >
        Show more
      </button>
    </div>)
