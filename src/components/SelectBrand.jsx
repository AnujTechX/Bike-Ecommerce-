...
import SelectBrandCards from "./SelectBrandCards"
  ...
  const {data:bikes}= useGetBikesQuery()
   ...
   useEffect(()=>{
    const getBrandItems=()=>{
      ...
      const bItems= bikes?.filter((item)=> 
      item.brand.toLowerCase()===brand.toLowerCase())
      setSelectedBrands(bItems)
      console.log("brand name ",brand.toLowerCase())
      console.log("brand items ",bItems)
    }
    getBrandItems()
   },[bikes,brand])
        ...
        <div className="mb-4 mx-2 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3">
         ...
         selectedBrands &&
            selectedBrands?.map((item,index)=>
             <SelectBrandCards item={item} key={index}/>)
         }
